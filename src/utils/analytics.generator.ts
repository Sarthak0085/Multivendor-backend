...
export async function generateLast12MonthsData<T extends Document>(
    model: Model<T>
): Promise<{ analyticsData: MonthData[] }> {
...
export async function generateLast30DaysData<T extends Document>(
    model: Model<T>
): Promise<{ analyticsData: DayData[] }> {
    const analyticsData: DayData[] = [];
    const currentDate = new Date();
...
export async function generateLast24HoursData<T extends Document>(
    model: Model<T>
): Promise<{ analyticsData: HourlyData[] }> {
    const analyticsData: HourlyData[] = [];
    const currentDate = new Date();
